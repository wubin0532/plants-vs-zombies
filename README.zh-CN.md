...
不登录时游戏完全离线，存档仍只在本机；登录后进度随账号保存到服务器，可跨设备继续。同一台手机上访客与各账号的本机档互相隔离，新账号不会继承上一个人的进度。
...
- 切到后台自动暂停。全屏战斗时种子栏浮在棋盘左侧留白、进度与计时压成顶部细条（暂停时才显示“休息一下”）；手机竖屏会给出旋转引导（可跳过），iPhone 可用“添加到主屏幕”进入无地址栏的独立窗口；独立窗口下主页顶栏会自动让开状态栏并吸顶；手机横屏时会自动进入战斗全屏（主动退出后本次横屏不再自动进入，转回竖屏再横屏或开新一局恢复）。
...
- 多发/连发与黄油已按真实单颗弹丸结算（双发/机枪每颗 20、香蒲每枚 25，黄油为玉米粒伤害的 2 倍且在发射前确定）；杨桃为五向固定星星，投掷弹道仍为简化机制；原版个别细节仍需逐项对照。
...
- 后端测试 `server/index.test.mjs` 覆盖注册、密码校验、未登录 401、登出撤销会话、乐观并发、**存档双向隔离（接口与落盘）**、跨站写拒绝、限流与 `MAX_USERS` 上限；前端有云存档合并、导入校验与**同机多账号存档隔离**测试。精灵切图的两张源图已纳入版本库（见 `.gitignore`），`npm test` 的通过数在干净克隆 / CI 上可复现（当前 598 项）。
...
- `src/store.ts`、`src/save-keys.ts`：Pinia 与带校验的 localStorage 存档；本机档按访客/账号分槽，登录后叠加防抖云同步。
...
本机存档按"归属"分槽：未登录为 `pvz-garden-save-v1::guest`，登录后为 `pvz-garden-save-v1::<用户 id>`，旧版全局键 `pvz-garden-save-v1` 在首次加载时迁移为访客档。因此同一台手机上切换账号不会互相继承进度；新账号云端为空且本机访客档有进度时，会弹出确认框，默认**不带入**，只有玩家主动选择才合并。存档版本 2，兼容迁移版本 1。导入只接受连续、合法的关卡进度，失败不会覆盖当前数据。清理站点数据、更换浏览器、域名或端口会影响存档访问，应提前导出 JSON 备份。登录或会话恢复后会先取云端并做**进度并集合并**（关卡进度/金币/成就/星级等取并集，绝不丢关卡进度），再用**服务端修订号**做乐观并发写入：冲突时合并后重传，不再用客户端时钟对账。设置页仍提供手动"上传/下载覆盖"按钮。
