...
- 切到后台自动暂停。全屏战斗时种子栏浮在棋盘左侧留白、状态栏浮在底部；手机竖屏会给出旋转引导（可跳过），iPhone 可用“添加到主屏幕”进入无地址栏的独立窗口。
...
- 后端测试 `server/index.test.mjs` 覆盖注册、密码校验、未登录 401、存档按用户隔离与 `MAX_USERS` 上限；`npm test` 共 576 项通过。
...
移动端最大化：战斗沉浸布局改为“战场铺满 + 种子栏/状态栏浮层”，加入竖屏旋转引导、横竖屏与地址栏收放后的画布重算，以及 PWA manifest 与 iOS 独立窗口 meta（`scripts/prepare-icons.mjs` 生成主屏幕图标）。
