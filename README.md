...
- Backend tests in `server/index.test.mjs` cover registration, password verification, 401 when logged out, logout revoking sessions, optimistic concurrency, **save two-way isolation (API and on-disk)**, cross-site write rejection, rate limiting, and the `MAX_USERS` cap; the frontend has tests for cloud-save merging, import validation, and **same-device multi-account save isolation**. The two sprite source sheets are committed to the repo (see `.gitignore`); the `npm test` pass count is reproducible on a clean clone / CI (currently 598).
